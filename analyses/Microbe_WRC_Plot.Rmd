...
x <- WRC.bac.in2[1:5, 1:5] #just a test
...
y <- WRC.bac.REL[1:5, 1:5] #just a test
y

# Alternative Function for Calculating Relative Abundances
WRC.bac.REL.log <- decostand(WRC.bac, method="total")
z <- WRC.bac.REL.log[1:5, 1:5] #just a test
z
...
explainvar1 <- round(WRC.bac.pcoa$eig[1] / sum(WRC.bac.pcoa$eig), 2) * 100
explainvar2 <- round(WRC.bac.pcoa$eig[2] / sum(WRC.bac.pcoa$eig), 2) * 100
...
## PCoA Plot - plot function (ggplot2 version below)
```{r, results=hide}
...
pch.col <- c("gray90", "gray90", "gray90", "gray90", "gray90", 
              "forestgreen", "forestgreen", "forestgreen", 
              "forestgreen", "forestgreen")
bg.col <- c("gray10", "gray10", "gray10", "gray10", "gray10",
            "gray10", "gray10", "gray10", "gray10", "gray10")

pch.loc <- c(22, 24, 22, 24, 21, 22, 24, 22, 25, 21)

# then label AV or EC
# since plant doesn’t seem to mean as much (??)

# Define Plot Parameters
#png(filename="../figures/Microbe_PCoA.png",
#      width = 1200, height = 800, res = 96*2, bg = "white")
pdf(file="../figures/Microbe_PCoA.pdf",
      width = 6, height = 4, bg = "white")
par(opar)
layout(matrix(1:2, 1), widths = c(5, 2))
par(mar = c(5, 5.5, 1, 1) + 0.1)

plot(cent.dataframe[,1], cent.dataframe[,2], type = 'n', las = 1,
     xlim = c(-0.25, 0.25), ylim = c(-0.3, 0.2),
...
points(cent.dataframe[,1], cent.dataframe[,2], 
       cex = 2.5, bg = pch.col, col = bg.col, pch = pch.loc, lwd = 2)
...
legend(0, 1, legend = c("Bulk", "Rhizosphere", "Rhizoplane"),
       col = "gray10", pt.bg = c("white", "white", "white"), 
       pch = c(21, 24, 22), cex = 0.75, bty = 'n', pt.cex = c(1, 0.75, 0.75), 
       y.intersp = 1.5, pt.lwd = 1)

legend(0, 0.6, legend = c("Fertilized", "Unfertilized"),
       col = "gray10", pt.bg = c("forestgreen", "gray90"), 
       pch = c(22, 22), cex = 0.75, bty = 'n',
       y.intersp = 1.5, pt.lwd = 1)

dev.off() # this writes plot to folder
graphics.off() # shuts down open devices

#img <- readPNG("../figures/Microbe_PCoA.png")
#grid.raster(img)
```
...
```{r}
pdf(file="./figures/Microbe_PCoA_TrtYr.pdf",
...
## PerMANOVA
```{r}
adonis(WRC.bac.REL ~ Treatment + Plant + Location, data = design, strata = design$Treatment, perm = 999)
...
```{r}
# Import Raw Data
raw <- WRC.bac.in <- read.delim("../data/WRC2014_bulk.shared")
WRC2014 <- raw[, -1]
rownames(WRC2014) <- raw[, 1]

WRC2014[1:5, 1:5]

design <- as.data.frame(matrix(NA, ncol = 3, nrow = dim(WRC2014)[1]))
colnames(design) <- c("sample", "treatment", "replicate")
design$sample <- rownames(WRC2014)
design$treatment <- gsub("^[0-9]", "", sapply(strsplit(design$sample, "-"), "[", 2))
design$replicate <- gsub("[A-Z].*$", "", sapply(strsplit(design$sample, "-"), "[", 2))

# Remove OTUs with less than two occurences across all sites
WRC.bac <- WRC2014[, which(colSums(WRC2014) >= 2)]
...
```{r}
pdf(file="./figures/Microbe_PCoA_Treatment.pdf",
    width = 9, height = 3, bg = "white")
